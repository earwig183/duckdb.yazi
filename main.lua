...
local function remove_file(cache_url)
  if fs.cha(cache_url) then
    ...
    if not ok then
      ya.err(
        string.format(
          "[duckdb] failed to remove partial cache at %s: %s",
          tostring(cache_url),
          tostring(err)
        )
      )
...
local function is_plain_text(job, file_type)
  ...
  local query = {
    ...
    ".headers off",
    string.format(
      "select count(column_name) from (describe from read_csv('%s'));",
      tostring(job.file.url)
    ),
  }
  local output = run_query(job, query, nil, file_type)
...
function M:peek(job)
  ...
  local query =
    generate_peek_query(args.target, job, args.limit, args.offset, args.file_type, args.cache_str)
